...
# ===============================
# THE REPAIRED CSS (STRICT FRONT-END)
# ===============================
st.markdown("""
<style>
    ...
    /* GAP: Separation between header and first data row */
    div[data-testid="stHorizontalBlock"]:has(> div:nth-child(4):last-child):nth-of-type(2) {
        margin-top: 5px !important;
    }

    /* BUTTONS: Force identical full-width on all types */
    .stButton > button {
    ...
    /* 4-Hour Block Colors */
    .time-block-0 { background-color: #f1f3f5 !important; } 
    .time-block-1 { background-color: #e9ecef !important; } 
    .time-block-2 { background-color: #dee2e6 !important; } 
    .time-block-3 { background-color: #ced4da !important; } 
    .time-block-4 { background-color: #adb5bd !important; } 
...
tab_booking = st.tabs(["🎱 Bookings", "⚙️ Admin"])[0] if st.session_state.role == "admin" else st.tabs(["🎱 Bookings"])[0]
...
with tab_booking:
    ...
    for t in times:
        r_cols = st.columns(4)
        block_idx = (int(t.split(":")[0]) - 6) // 4 # Change color every 4 hours
