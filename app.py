...
# 4. COMPACT CSS (SHRUNK HEIGHT & NARROW COLUMNS)
st.markdown("""
<style>
/* FORCE HORIZONTAL - STOP STACKING */
...
/* NARROW DATA COLUMNS */
.data-row [data-testid="column"]:nth-child(1) { flex: 0 0 45px !important; min-width: 45px !important; }
.data-row [data-testid="column"]:not(:nth-child(1)) { flex: 0 0 60px !important; min-width: 60px !important; }

/* DATE GRID: 7 WIDE */
.date-grid [data-testid="column"] { flex: 0 0 42px !important; min-width: 42px !important; }

/* SHRUNK BUTTONS (Low Highness) */
.stButton > button {
    width: 100% !important;
    height: 32px !important; /* Shrunk height */
...
.header-box {
    ...
    font-size: 8px; padding: 2px 0; border-radius: 2px;
    width: 60px;
}
.time-label { font-size: 11px; font-weight: bold; text-align: center; line-height: 32px; }
...
/* KILL PADDING */
[data-testid="stAppViewBlockContainer"] { padding: 0.5rem 0.1rem !important; }
</style>
...
if st.session_state.pending_cancel:
    ...
    if st.button(f"CONFIRM CANCEL: {b_name}", type="primary"):
        bookings = bookings.drop(idx); save_data(bookings, BOOKINGS_FILE)
        st.session_state.pending_cancel = None; st.rerun()
    if st.button("Back"): st.session_state.pending_cancel = None; st.rerun()
    st.stop()
    ...
    for i in range(3):
        ...
        with r_cols[i+1]:
            if not match.empty:
                ...
                btn_label = f"❌ {b_name[:4]}"
                if st.session_state.role == "admin" or b_user == st.session_state.user:
                    if st.button(btn_label, key=f"b_{t}_{i}"):
                        st.session_state.pending_cancel = (match.index, b_name); st.rerun()
                else: st.button(f"🔒 {b_name[:4]}", key=f"b_{t}_{i}", disabled=True)
            else:
