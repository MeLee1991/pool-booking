...
<style>
    ...
    /* 2. MAIN TABLE - Strict 4-Column Grid Alignment */
    .table-wrapper div[data-testid="stHorizontalBlock"] {
        display: grid !important;
        grid-template-columns: repeat(4, 1fr) !important; /* Forces buttons = header width */
        gap: 4px !important;
    ...
    .stButton > button {
        width: 100% !important;
        height: 44px !important; /* Fixed height matching header */
...
# ===============================
# DATE SELECTOR (1.5x Wider Buttons)
# ===============================
...
for i in range(7):
    ...
    d_cols1[i].button(lbl, key=f"date_{d}", type="primary" if d == st.session_state.sel_date else "secondary", on_click=set_date, args=(d,))
...
for i in range(7, 14):
    d = dates[i]
    lbl = f"{d.strftime('%a').upper()}\n{d.day}"
    d_cols2[i-7].button(lbl, key=f"date_{d}", type="primary" if d == st.session_state.sel_date else "secondary", on_click=set_date, args=(d,))

st.divider()

# ===============================
# MAIN TABLE (Wrapped for strict CSS)
# ===============================
