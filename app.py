...
<style>
    ...
    /* The 4-Column Grid (Time, T1, T2, T3) */
...
def handle_booking(date_str, table, time_str, user_email, role):
    # Admin power: uses target user from dropdown
...
if "user" not in st.session_state:
    ...
    if m == "Login":
        ...
        ru, rp = st.text_input("New Username").lower(), st.text_input("New Password", type="password")
...
if t_admin:
    with t_admin:
        ...
        edited = st.data_editor(udf, use_container_width=True, num_rows="dynamic", column_config={
            "role": st.column_config.SelectboxColumn("Role", options=["user", "admin"]),
            "approved": st.column_config.CheckboxColumn("Approved")
        })
        if st.button("💾 Save User Changes"):
            save_data(edited, USERS_FILE); st.success("Changes saved!"); st.rerun()
        
        st.divider()
        st.subheader("Admin Booking Tool")
        ulist = udf["email"].astype(str).tolist()
        st.session_state.admin_target_user = st.selectbox("Assign bookings to:", ulist, index=ulist.index(st.session_state.user))

# Bookings Logic (Exact Grid)
with t_book:
        ...
        for i, tab in enumerate(["T1", "T2", "T3"]):
            with rcols[i+1]:
                ...
                if not m.empty:
                    ...
                    # Users see a lock if they can't delete it
                    can_del = owner.lower() == st.session_state.user.lower() or st.session_state.role == "admin"
                    btn_text = f"X {disp}" if can_del else f"🔒 {disp}"
                    st.button(btn_text, key=k, type="primary", on_click=handle_booking if can_del else None, args=(str(st.session_state.sel_date), tab, t, st.session_state.user, st.session_state.role))
                else:
