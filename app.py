...
def load_data(file, cols):
    if not os.path.exists(file):
        ...
        return df
    df = pd.read_csv(file)
    # FIX: If columns are missing/wrong, reset to avoid KeyErrors
...
if "user" not in st.session_state:
    ...
    if mode == "Login":
        ...
        if st.button("Submit", use_container_width=True):
            u_df = load_data(USERS_FILE, USER_COLS)
            if r_email in u_df["email"].values: st.error("Already exists.")
            else:
...
# MAIN TABS
# ===============================
t_list = ["🎱 Bookings", "👤 Profile"]
if st.session_state.role == "admin": t_list.append("⚙️ Admin")
tabs = st.tabs(t_list)
        ...
        for i, table in enumerate(["T1", "T2", "T3"]):
            with r_cols[i+1]:
                ...
                if not match.empty:
                    ...
                    if st.button(disp, key=btn_key, type="primary", use_container_width=True):
...
if st.session_state.role == "admin":
    with tabs[2]:
        ...
        if not bookings.empty:
            stats = bookings.groupby("user").size().reset_index(name="Bookings").sort_values("Bookings", ascending=False)
