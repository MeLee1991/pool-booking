...
if st.session_state.user is None:
    ...
    if st.button("Login", use_container_width=True):
        match = st.session_state.users[(st.session_state.users["Email"]==e) & (st.session_state.users["Password"]==p)]
        if not match.empty:
...
# This handles the clicks sent from the HTML without using URLs
...
for i in range(14):
    ...
    date_cells += f'<div class="{cls}" onclick="stClick(\'date\', \'{d_str}\')">{label}</div>'
...
for idx, t in enumerate(HOURS):
    ...
    for i in range(1, 4):
        ...
        if not match.empty:
            ...
            grid_rows += f'<div class="cell {"mine" if is_mine else "taken"}" onclick="{"stClick(\'del\',\''+t+'|'+table+'\')" if is_mine else ""}">{"✕" if is_mine else ""}{b_name}</div>'
        else:
            grid_rows += f'<div class="cell free" onclick="stClick(\'book\',\'{t}|{table}\')">+</div>'
...
<style>
    * {{ box-sizing:border-box; margin:0; padding:0; font-family:sans-serif; }}
    ...
    <script>
        function stClick(act, val) {{
            // Use a Form to force the parent page to update. This is the most robust bridge.
            const url = new URL(window.parent.location.href);
            url.searchParams.set('a', act);
...
# This renders the grid and catches actions
components.html(html_code, height=1550, scrolling=False)

# One more catch for the URL params after the component renders
if "a" in st.query_params or "v" in st.query_params:
    process_command({"a": st.query_params.get("a"), "v": st.query_params.get("v")})
